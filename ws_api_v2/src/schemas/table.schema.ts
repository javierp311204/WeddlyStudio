...
export const listTablesSchema = z.object({
  ...
  query: z.object({
    include_guests: z.string().optional().transform(v => v !== 'false'), // default true
  }),
});
...
export const createTableSchema = z.object({
  ...
  body: z.object({
    ...
    shape: z.enum(['round', 'rectangular'] as const, {
      message: 'Shape inválido: round | rectangular',
    }).optional().default('round'),
    max_capacity: z.number().int().min(1, 'La capacidad mínima es 1').max(50).optional().default(10),
    pos_x: z.number().min(0).max(100).optional().default(50),
    pos_y: z.number().min(0).max(100).optional().default(50),
  }),
});
